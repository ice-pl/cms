...
// create new table galleryprops and fill it for every subgallery
  ...
  case "1.6":
  case "1.6.1":
    @unlink($this->GetModuleURLPath() . '/function.upload.php');
    ...
    {
      @unlink($this->GetModuleURLPath() . '/templates/' . $deletefile);
    }
    @rmdir($this->GetModuleURLPath() . '/templates/jquery');
    @rmdir($this->GetModuleURLPath() . '/templates/swfupload');
    ...
    foreach ($deletefiles as $deletefile)
    {
      @unlink($this->GetModuleURLPath() . '/lib/swfupload/' . $deletefile);
    }
    @rmdir($this->GetModuleURLPath() . '/lib/swfupload');
    $deletefiles = array('error.gif', 'SmallSpyGlassWithTransperancy_17x18.png', 'toobig.gif', 'uploadlimit.gif', 'zerobyte.gif');
    foreach ($deletefiles as $deletefile)
    {
      @unlink($this->GetModuleURLPath() . '/images/' . $deletefile);
    }
  ...
  case "2.0.1":
    ...
    foreach (glob($this->GetModuleURLPath() . '/templates/css/*.css') as $filename)
    {
