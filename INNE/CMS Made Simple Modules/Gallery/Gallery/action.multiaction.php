...
switch ($params['multiaction'])
{
  ...
  case 'rotateanticlockwise': {
      if ($permission_to_edit == 'all' || $permission_to_edit == 'this')
      {
        ...
        {
          $fileinfo = Gallery_utils::Getgalleryinfobyid($fid);
          $image = Gallery_utils::DefaultGalleryPath() . $fileinfo['filepath'] . $fileinfo['filename'];
  ...
  case 'move': {
      ...
      if (Gallery_utils::CheckEditor($userid, $params['moveto'], $editors))
      {
        ...
        foreach ($fid_array as $fid)
        {
          ...
          {
            $fileinfo = Gallery_utils::Getgalleryinfobyid($fid);
            $newpath = Gallery_utils::DefaultGalleryPath() . $newdir . $fileinfo['filename'];
            $oldpath = Gallery_utils::DefaultGalleryPath() . $fileinfo['filepath'] . $fileinfo['filename'];
            if (strpos($fileinfo['filename'], "/") === FALSE)
            {
              ...
              if ($permission_to_edit == 'all' || $permission_to_edit == 'this')
              {
                ...
                {
                  $newpath = Gallery_utils::DefaultGalleryPath() . $newdir . IM_PREFIX . $fileinfo['filename'];
                  $oldpath = Gallery_utils::DefaultGalleryPath() . $fileinfo['filepath'] . IM_PREFIX . $fileinfo['filename'];
